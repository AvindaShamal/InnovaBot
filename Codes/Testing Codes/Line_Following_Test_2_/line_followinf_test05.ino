...
int IR_weight[8] = {-40, -30, -10, -5, 5, 10, 30, 40};
...
// Add a flag to indicate a sharp turn
...
void loop() {
  ...
  // Check for a sharp turn
  if (isSharpTurn) {
    delay(800); // Add a delay for better handling of sharp turns
    isSharpTurn = false; // Reset the sharp turn flag
  }
}
...
float calculatePID(int *sensorValues) {
  ...
  // Check for a sharp turn
  if (onLine >= 6) {  // Tweak the threshold based on your sensor layout
    isSharpTurn = true;
  }
...
void PID_Linefollow(float pidValue) {
  ...
  if (isSharpTurn) {
    // Reduce speed and adjust turn for sharp turns
    speed = lfspeed / 6;
    pidValue *= 1.2;  // Tweak the multiplier based on experimentation
  }
