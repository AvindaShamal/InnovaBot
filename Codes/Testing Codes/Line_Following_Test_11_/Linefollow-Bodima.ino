...
void turnLeft(int *sensorValues) {
  ...
  while (!enoughturn && !whitedetect) {
    ...
    motor2.setSpeed(85);   // Set right motor speed
    motor2.forward();       // Move right motor forward
    motor1.setSpeed(85);   // Set left motor speed
...
void turnRight(int *sensorValues) {
  ...
  while (!enoughturn && !whitedetect) {
    ...
    motor2.setSpeed(85);   // Set right motor speed
    motor2.backward();       // Move right motor forward
    motor1.setSpeed(85);   // Set left motor speed
    motor1.forward();      // Move left motor backward for a sharper turn
