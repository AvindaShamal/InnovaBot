...
void loop() {
  ...
  for (int i = 0; i < numSensors; i++) {
    Serial.print(IR_value[i]); // Print the sensor values to the serial monitor
  ...
  if (IR_val[0] == 0 && IR_val[1] == 0 && IR_val[2] == 0 && IR_val[3] == 0 && IR_val[4] == 0 && IR_val[5] == 0 && IR_val[6] == 0 && IR_val[7] == 0) {
    // Stop the motors when all sensors detect white
    stop();
  } else {
    PID_control();
    set_speed();
...
void PID_control() {
  ...
  LMotorSpeed = 60 - speedAdjust;
  RMotorSpeed = 60 + speedAdjust;
